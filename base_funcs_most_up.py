...
def config_read(path):
    ...
    dict_config["r_cutoff"] = float(dict_config["r_cutoff"]) * const.R_sun
    ...
    if dict_config["save_each"] == "True":
        dict_config["save_each"] = True
    else:
        dict_config["save_each"] = False
...
def generate_temp_arr(config):  # ask if len r will be user defined
    ...
    t_visc = ma.masked_less(t_visc, 1400 * u.K)  # 1400 K taken to be the sublimation temperature of dust
    ...
    for i in range(len(r_visc)):
        ...
        elif t_int > 70 and t_int % 2 == 1:  # As per temperatures in BT-Settl data
...
def magnetospheric_component(config, r_in):
    ...
    if config['mag_comp'] == "hslab":
        # retrieve stored H-slab SEDs
        h_flux = np.load(f"{config['h_data']}/temp_{int(t_slab.value)}_tau_{np.round(tau, 1)}/Flux_wav.npy")
        h_minus_flux = np.load(f"{config['h_min_data']}/{int(t_slab.value)}_tau_{np.round(tau, 1)}_e"
                               f"{int(np.log10(n_e.value))}/Flux_wav.npy")
    ...
    if config['mag_comp'] == "hslab":
        ...
        # calculate blackbody spectrum, defined at 8000 K
        scale_unit = u.erg / (u.cm ** 2 * u.s * u.AA * u.sr)
        bb = BlackBody(8000 * u.K, scale=1 * scale_unit)
        wavelength = np.logspace(np.log10(l_min.value), np.log10(l_max.value), n_data) * u.AA
...
def generate_dusty_disk_flux(config, r_in, r_sub):
    ...
    r_dust = np.logspace(np.log10(r_sub.si.value), np.log10(270 * const.au.value), 2000) * u.m
